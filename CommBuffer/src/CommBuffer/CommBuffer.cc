...
**
*/
bool XMLWriteBegin(ostream& out,const char *key)
{
  out << "<";
  while(*key != '\0')
      out.put(*key++);
...
**
*/
bool XMLWriteEnd(ostream& out,const char *key)
{
  out.flush();
  out << "<\\";
  while(*key != '\0')
...
**
*/
bool XMLReadBegin(istream& in,char *key)
{
  bool ans = true;
  char c = in.get();
  if(c == '<')
...
**
*/
bool XMLReadEnd(istream& in,char *key)
{
  bool ans = true;
  char c = in.get();
  if(c == '<')

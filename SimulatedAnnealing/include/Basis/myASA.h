...
***********************************************************************/
 ...
 /* $Id: myASA.h,v 1.1 2008/02/17 12:13:29 blurock Exp $ */

 /* asa.h for Adaptive Simulated Annealing */

#include "myASA_user.h"
...
#define ROW_COL_INDEX(i, j) ((i) + *number_parameters * (j))
 ...
 /* INVALID_COST_FUNCTION test */
#define COST_FUNCTION_TEST(x) ((x) != (x) || x < -MAX_DOUBLE || x > MAX_DOUBLE)
...
*/
/*
void SimulatedAnnealing::cost_derivatives (double (*user_cost_function) (double *),
		       double *parameter_minimum,
		       double *parameter_maximum,
		       double *tangents,
		       double *curvature,
...
double generate_asa_state (double (*user_random_generator) (LONG_INT *),
			   ...
			   double *temp);
/*
void SimulatedAnnealing::asa_exit (double (*user_cost_function) (double *),
	       double *final_cost,
...
#endif /* TIME_STD */
#else /* TIME_GETRUSAGE */
void aux_print_time (clock_t time, char *message,
		     FILE * ptr_asa_out);
